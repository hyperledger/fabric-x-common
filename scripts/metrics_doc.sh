...
generate_doc() {
    ...
    local orderer_deps=()
    while IFS= read -r pkg; do orderer_deps+=("$pkg"); done < <(go list -deps github.ibm.com/decentralized-trust-research/fabricx-config/cmd/orderer | sort -u | grep -E "(hyperledger|hyperledger-labs/SmartBFT)")
    ...
    local peer_deps=()
    while IFS= read -r pkg; do peer_deps+=("$pkg"); done < <(go list -deps github.ibm.com/decentralized-trust-research/fabricx-config/cmd/peer | sort -u | grep hyperledger)
