...
func TestBlockSignatureVerifierByIdentifier(t *testing.T) {
	...
	require.Equal(t, applicationpb.NewIdentityWithCertificate("msp1", []byte("identity1")), signatureSet[0].Identity)
	require.Equal(t, applicationpb.NewIdentityWithCertificate("msp3", []byte("identity3")), signatureSet[1].Identity)
}
...
func TestBlockSignatureVerifierByCreator(t *testing.T) {
	...
	idty := applicationpb.NewIdentityWithCertificate("msp1", []byte("creator"))
	header := &cb.BlockHeader{}
	md := &cb.BlockMetadata{
		Metadata: [][]byte{
			protoutil.MarshalOrPanic(&cb.Metadata{Signatures: []*cb.MetadataSignature{
