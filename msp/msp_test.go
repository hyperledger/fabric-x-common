...
func TestDeserializeIdentityFails(t *testing.T) {
	_, err := localMsp.DeserializeIdentity(applicationpb.NewIdentityWithCertificate("SampleOrg", []byte("barfr")))
	require.Error(t, err)

	_, err = localMsp.DeserializeIdentity(applicationpb.NewIdentityWithCertificate("SampleOrg", []byte(notACert)))
	require.Error(t, err)
}
...
func TestIdentityPolicyPrincipalFails(t *testing.T) {
	...
	sid, err := NewSerializedIdentityWithCert("SampleOrg", []byte(othercert))
	require.NoError(t, err)
