...
func TestLoadPrivateKey(t *testing.T) {
	t.Parallel()
	testDir := t.TempDir()
	priv, err := cryptogen.GeneratePrivateKey(testDir, cryptogen.ED25519)
	require.NoError(t, err, "failed to generate private key")
	pkFile := filepath.Join(testDir, "priv_sk")
	require.FileExists(t, pkFile, "Expected to find private key file")
	loadedPriv, err := cryptogen.LoadPrivateKey(testDir)
...
func TestLoadPrivateKey_BadPEM(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			_, err = cryptogen.LoadPrivateKey(badPEMFile)
...
func TestGeneratePrivateKey(t *testing.T) {
	...
	priv, err := cryptogen.GeneratePrivateKey(testDir, cryptogen.ECDSA)
	require.NoError(t, err, "Failed to generate private key")
	...
	_, err = cryptogen.GeneratePrivateKey("notExist", cryptogen.ECDSA)
...
func TestECDSASigner(t *testing.T) {
	...
	// unmarshal signature
	ecdsaSig := &cryptogen.ECDSASignature{}
...
func TestED25519Signer(t *testing.T) {
	...
	signer := cryptogen.ED25519Signer{
		PrivateKey: priv,
	}
