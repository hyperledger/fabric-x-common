...
func (t *MspTree) generateTLS(p NodeParameters) error {
	...
	switch p.Type {
	case NodeTypeClient, NodeTypeAdmin:
...
func getNodeOU(nodeType int) []string {
	switch nodeType {
	case NodeTypeClient:
		return []string{clientOU}
	case NodeTypePeer:
		return []string{peerOU}
	case NodeTypeAdmin:
		return []string{adminOU}
	case NodeTypeOrderer:
		return []string{ordererOU}
	default:
		return []string{}
	}
}
