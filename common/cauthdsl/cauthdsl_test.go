...
var signers = []*applicationpb.Identity{
	applicationpb.NewIdentityWithCertificate("org1", []byte("signer0")),
	applicationpb.NewIdentityWithCertificate("org1", []byte("signer1")),
}
...
func TestComplexNestedSignature(t *testing.T) {
	...
	if !spe(ToIdentities(append(signers, applicationpb.NewIdentityWithCertificate("org1", []byte("signer0"))),
	...
	if spe(ToIdentities(append(signers, applicationpb.NewIdentityWithCertificate("org1", []byte("signer1"))),
		&MockIdentityDeserializer{})) {
