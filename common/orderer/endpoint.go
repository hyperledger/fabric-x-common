...
type (
	// Endpoint defines an orderer party's endpoint.
	Endpoint struct {
...
func ParseOrdererEndpoint(valueRaw string) (*Endpoint, error) {
	ret := &Endpoint{ID: NoID}
	if len(valueRaw) == 0 {
		return ret, nil
	}
...
func unmarshalOrdererEndpoint(valueRaw string, out *Endpoint) error {
