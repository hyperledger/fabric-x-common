...
func TestNewComparablePrincipal(t *testing.T) {
	...
	t.Run("Identity", func(t *testing.T) {
		expectedPrincipal := &ComparablePrincipal{
			mspID:    mspID,
			identity: applicationpb.NewIdentityWithCertificate(mspID, []byte("identity")),
			principal: &msp.MSPPrincipal{
				PrincipalClassification: msp.MSPPrincipal_IDENTITY,
				Principal: protoutil.MarshalOrPanic(
					applicationpb.NewIdentityWithCertificate(mspID, []byte("identity"))),
			},
		}
...
func identity(orgName string, idBytes []byte) *msp.MSPPrincipal {
	return &msp.MSPPrincipal{
		PrincipalClassification: msp.MSPPrincipal_IDENTITY,
		Principal:               protoutil.MarshalOrPanic(applicationpb.NewIdentityWithCertificate(orgName, idBytes)),
	}
}
