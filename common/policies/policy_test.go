...
func TestSignatureSetToValidIdentities(t *testing.T) {
	id := applicationpb.NewIdentityWithCertificate("org1", []byte("identity1"))
...
func TestSignatureSetToValidIdentitiesDeserializeErr(t *testing.T) {
	...
	invalidCertIdentity := applicationpb.NewIdentityWithCertificate("org1", []byte("identity1"))
	tests := []struct {
		spec                     string
		...
		},
		{
			...
			signedData: []*protoutil.SignedData{
				{
					Identity: applicationpb.NewIdentityWithCertificate("org1", client1.Cert),
				},
			},
			expectedLogEntryContains: []string{
				"invalid identity",
