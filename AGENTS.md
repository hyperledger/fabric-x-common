...
Requires the Go toolchain (`go 1.27`) plus dev tools from `scripts/install-dev-dependencies.sh`
...
make mocks        # regenerate counterfeiter fakes (go generate ./...) + mockery mocks (.mockery.yml)
```
...
5. **Mocks are generated.** counterfeiter fakes come from `//go:generate counterfeiter ...`
   directives; mockery mocks are configured in `.mockery.yml` (mockery v3 takes no per-interface
   flags). Both are regenerated by `make mocks` — don't edit them by hand.
