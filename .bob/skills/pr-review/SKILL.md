...
**How to check:** For each file ask: "If I reverted this file's changes, would the PR's stated goal still work?" If yes and not trivially related → scope creep. (A *trivially small* adjacent fix — e.g. a typo on a line you're already touching — is fine; reserve the flag for non-trivial unrelated changes.)
...
- **Merge conflicts**: Note in the summary that the branch needs a rebase/merge with `main`; still review the code as-is.
...
### Go Type Safety & Encapsulation

- ✅ Structs over `map[string]interface{}` when the keys are known at compile time; internal-only methods kept unexported; single-use helpers inlined into their caller.
- ✅ `errors.Wrapf` preserves the error chain where a cause exists (don't drop it with a fresh `errors.Newf`).
- ❌ Flag `map[string]interface{}` with a fixed key set; needlessly exported methods; single-call helpers that only add indirection.
...
- ✅ Functions read top-to-bottom; guard clauses / early returns for errors; helpers near their callers; nesting kept shallow (aligns with `gocognit` ≤ 15).
- ❌ Flag interleaved setup/logic/cleanup, ping-pong call patterns, and deep nesting that should be flattened.
...
Also review the *shape* of new proto, not only its compatibility:

- ✅ `oneof` when a field is one of several types (avoids re-marshalling through a `bytes` field); typed sub-messages over generic `bytes`; consistent field naming; flat hierarchy.
- ❌ Flag `bytes` fields that always carry a known message type, redundant/overlapping fields, and inconsistent naming.
...
### 8. Untrusted-Input Allocation Bounds

Block, tx, config, and ASN.1 decoders parse attacker-influenced bytes.

- ✅ Slice/map allocations sized from a decoded length or a repeated-field count are validated or bounded before `make([]T, n)`.
- ❌ Flag `make([]T, n)` or `append` loops where `n` comes straight from an untrusted protobuf count or ASN.1 length (unbounded allocation while parsing a block/tx/config).

## Performance / Benchmark Review

When a PR optimizes a hot path or adds benchmarks (ASN.1 encoding, hashing, proto marshalling), review the methodology, not just the reported numbers:

- ✅ Before/after numbers shown; `b.ReportAllocs()`; realistic, varied inputs (not the same value every iteration); a baseline to compare against.
- ❌ Flag benchmarks that reuse identical data every iteration, missing allocation tracking, and optimizations claimed without benchmark evidence.
...
encoding choices, deviations from upstream Fabric. The best ones also record the alternatives
considered and the trade-off that made this approach win. Flag "what" comments that add nothing.
...
- [ ] Compliance: error handling, simplicity, type safety, linear flow, naming, license headers, fork discipline
...
- [ ] Untrusted-input allocation bounds checked; benchmark methodology verified (if perf/benchmarks touched)
