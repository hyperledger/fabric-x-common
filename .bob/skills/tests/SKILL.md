...
Building a **fresh fixture per case** — a new `tlsgen.NewCA()` or a new config block — is fine and
often clearer than sharing one mutable fixture across cases; it also avoids data races under
`t.Parallel()`.
...
separate success vs failure loops with a comment when their fields differ. Keep cases
**declarative**: plain data fields (inputs, expected values, error substrings). Don't put
function/callback fields in the case struct — branch on a data field instead, so cases stay
...
- Use `require.Eventually` / `require.EventuallyWithT` for async conditions — **never `time.Sleep`**.
  Reach for **`require.EventuallyWithT`** whenever the wait checks more than one thing (or a
  non-boolean condition): its callback receives a `require.TestingT`, so a timeout reports exactly
  which sub-assertion failed — including `test.RequireProtoEqual` and friends. `require.Eventually`
  takes a single `bool` and can only tell you the whole wait timed out.
