...
- **mockery** (v3) generates the `core/policy/mocks/` testify mocks. It is invoked by a single bare
  `//go:generate mockery` (v3 takes no per-interface flags); which interfaces it covers is set in
  `.mockery.yml` at the repo root. Also regenerated by **`make mocks`**.
